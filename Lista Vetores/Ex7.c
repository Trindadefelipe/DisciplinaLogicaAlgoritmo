...
int main() {
    ...
    int maior_dia_prod = 0, idx_maior_dia = 0;
    int dias_baixa_prod = 0;

    printf("=== Problema 7: Controle de Producao ===\n");
    
    printf("Digite a producao diaria (7 dias) do turno MANHA:\n");
    for(i=0; i<7; i++) { printf("%s: ", dias[i]); scanf("%d", &manha[i]); total_manha += manha[i]; }

    printf("Digite a producao diaria (7 dias) do turno TARDE:\n");
    for(i=0; i<7; i++) { printf("%s: ", dias[i]); scanf("%d", &tarde[i]); total_tarde += tarde[i]; }

    printf("Digite a producao diaria (7 dias) do turno NOITE:\n");
    for(i=0; i<7; i++) { printf("%s: ", dias[i]); scanf("%d", &noite[i]); total_noite += noite[i]; }
    ...
    if(total_manha > total_tarde && total_manha > total_noite) printf("Turno Campeao: Manha\n");
    else if(total_tarde > total_noite) printf("Turno Campeao: Tarde\n");
    else printf("Turno Campeao: Noite\n");
